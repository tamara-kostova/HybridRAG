...
from db.db_client import QdrantWrapper

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)


class DocumentProcessorLangchain:
    def __init__(self, db_client: QdrantWrapper, model_name:str="all-MiniLM-L6-v2", directory_path:str=""):
        try:
    ...
    def process_pdf(self, pdf_path):
        try:
            ...
            if valid_chunks_and_embeddings:
                    self.db_client.insert_paper(pdf_filename, [chunk for chunk, _ in valid_chunks_and_embeddings],
                                                [emb for _, emb in valid_chunks_and_embeddings])
                    logger.info(f"Processed and inserted PDF {pdf_filename} into the database.")
            else:
