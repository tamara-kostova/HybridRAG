...
class DocumentProcessor:
    ...
    def process_paper(self, paper):
        try:
            ...
            valid_chunks_and_embeddings = [(chunk, emb) for chunk, emb in zip(chunks, embeddings) if emb is not None and len(emb) > 0]

            if not valid_chunks_and_embeddings:
                logger.warning(f"No valid embeddings generated for paper {paper['pmid']}")
                return None
