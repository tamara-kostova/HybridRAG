...
class NodeExtractor:
    def __init__(self, url:str = 'bolt://localhost:7687',username:str = 'neo4j',password = '',chunk_size:int = 1200,chunk_overlap:int = 200, embedding_model_name = "all-MiniLM-L6-v2"):
        try:
            self.graph = Neo4jGraph(url=url, username=username, password=password) 
        except Exception as e:
            logger.error(f'Unable to connect to Neo4j database: {e}')
            raise
        ...
        with open(file='src/graph/prompts/extraction_system_prompt.txt', mode='r') as file:
            prompt_content = file.read()
            self.system_prompt = prompt_content

        with open(file='src/graph/prompts/extraction_human_prompt.txt', mode ='r') as file: 
            prompt_content = file.read()
    ...
    def parse_extraction_output(self, response: str):
        ...
        if nodes_block:
            ...
            for line in nodes_lines:
                ...
                if match:
                    ...
                    embedding_string = node_id + " " + text 
                    
                    nodes[node_id] = Node(id=node_id, type=label, properties={"text": text, "embedding": self.embedding_model.encode(embedding_string)}) # TODO add embedding of text OR node id idk :( 
        ...
        if relationships_block:
            ...
            for line in relationships_lines:
                match = re.match(r'<"(.*?)", "(.*?)", "(.*?)", "(.*?)">', line.strip())
                if match:
                    node_id1, rel_type, rel_description, node_id2 = match.groups()
                    rel_embedding = self.embedding_model.encode(rel_description)
                    if node_id1 in nodes and node_id2 in nodes:
                        relationships.append(Relationship(
                            source=nodes[node_id1],
                            type=rel_type,
                            target=nodes[node_id2],
                            properties={"description": rel_description, "embedding": rel_embedding}
                        ))
    ...
    def extract_entities_from_pdf(
        ...
        for document in documents:
            ...
            for chunk in chunks:
                ...
                try:
                    response = chain.invoke(input={"input_chunk":chunk})

                except Exception as e:
                    print(f'Unable to process chunk, exception thrown: {e}')

                try:
                    extracted_nodes, extracted_relationships = self.parse_extraction_output(response.content)
